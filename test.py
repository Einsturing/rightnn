...
class Model(nn.Module):
    ...
    def test(self, inputs):
        ...
        # lpreds = torch.cat((linputs4, hinputs4), dim=1)
        lpreds1 = self.tconv4(hinputs1)
        # lpreds4 = self.unsample4(lpreds4)
        # lpreds3 = self.tconv3(lpreds4)
        # lpreds3 = self.unsample3(lpreds3)
        # lpreds2 = self.tconv2(lpreds3)
        # lpreds2 = self.unsample2(lpreds2)
        # lpreds1 = self.tconv1(lpreds2)
        # lpreds1 = self.unsample1(lpreds1)
        # out = self.loss(lpreds.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
        return hinputs1, lpreds1
