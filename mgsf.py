...
def Train():
    ...
    for idx, (inputs, tlabels) in enumerate(train_loader):
        ...
        loss = criterion(lpreds.reshape(-1), tlabels.reshape(multi, -1)[2])
...
if __name__ == '__main__':
    ...
    df = pd.read_csv('MG.csv').fillna(0).iloc[:, 3:].T.values
    ...
    train_set = inp[:80]
    test_set = inp[80 - (n_steps * jump + multi - 1):]
