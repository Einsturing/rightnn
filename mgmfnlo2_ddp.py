...
if __name__ == '__main__':
    ...
    train_loader = DataLoader(train, batch_size=1, sampler=train_sampler, shuffle=False, num_workers=8)
    test_loader = DataLoader(test, batch_size=1, sampler=test_sampler, shuffle=False, num_workers=8)

    model = Model()
    # model = nn.DataParallel(model, device_ids=[0])
    # model = model.cuda()
    model = nn.parallel.DistributedDataParallel(model.cuda(), device_ids=[args.local_rank], broadcast_buffers=False, find_unused_parameters=True)
